  ...
  return (
    ...
    >
      <div className="flex-0 min-w-8 flex items-center justify-center">
        <button
          className={clsx(
            "invisible group-hover:visible rounded bg-neutral-900",
            "text-neutral-400 hover:text-red-700 hover:bg-neutral-800 p-1"
          )}
          onClick={() => onDelete(id)}
          type="button"
        >
      ...
      <div className="flex-0 min-w-44 max-w-44 relative mx-2 min-h-8 flex items-center">
        <input
          type="text"
          className={clsx(
            "px-2 break-words whitespace-pre-wrap underline decoration-neutral-400 bg-neutral-900 w-full rounded-md",
            "outline-none shadow-outline",
            "focus:absolute focus:left-0 focus:py-1 focus:top-0 focus:cursor-auto focus:placeholder:visible focus:bg-neutral-700 focus:z-10",
        ...
        <div
          className={clsx(
            "absolute right-0 rounded-md bg-neutral-800 text-sm invisible text-neutral-400",
            "group-hover:visible",
      ...
      <div className="flex-1 text-sm flex items-center px-1 min-w-28">
        <div className="px-2">{formatDate(value.created_at)}</div>
      </div>
      <div className="flex-1 text-sm flex items-center px-1 min-w-28">
        <div className="px-2">{formatDate(value.update_at)}</div>
      </div>
    </div>
  );
}
        ...
        <div className="">
          <div className="m-2 flex items-center relative ">
            <button
              ...
              disabled={!isOK}
              className={clsx(
                "bg-neutral-800 rounded-md p-1 text-neutral-400",
                isOK
                  ? "hover:bg-neutral-700"
            ...
            >
              <div
                className={clsx(
                  isOK
                    ? "hidden"
                    : "invisible group-hover:visible absolute -top-9 bg-neutral-700 rounded-md w-64",
            ...
            <div className="relative flex-1">
              <input
                type="text"
                className={clsx(
                  "bg-neutral-800 ml-2 px-3 rounded-md accent-inherit outline-none shadow-outline text-sm text-neutral-300 placeholder:invisible",
                  "min-w-72",
                  "focus:absolute focus:left-0 focus:py-1 focus:-top-4 focus:cursor-auto focus:placeholder:visible",
                  "hover:cursor-pointer hover:absolute hover:left-0 hover:py-1 hover:-top-4 hover:placeholder:visible"
                )}
                placeholder="タイトルを入力"
                value={newTitle}
                onChange={(e) => {
        ...
        <div className="flex-1 flex-grow w-full flex flex-col justify-center items-center ">
          <div className="w-fit divide-slate-500 divide-y border-y">
            <div className="flex flex-nowrap space-x-1 divide-neutral-500 divide-x group h-auto text-neutral-200">
              ...
              <div className="flex-1 text-sm flex items-center px-1 min-w-28">
                <div className="px-2">作成日</div>
              </div>
              <div className="flex-1 text-sm flex items-center px-1 min-w-28">
                <div className="px-2">更新日</div>
              </div>
            </div>
            {pageList?.map((item) => (
              <Item
